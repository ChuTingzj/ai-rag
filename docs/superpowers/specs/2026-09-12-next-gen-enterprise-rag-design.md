...
1. M1 优先落地的 Wiki：Notion vs Confluence vs 飞书（按目标客户定一种）。  
2. 向量库默认选型（如 Qdrant / PGVector / 其他）与单机资源基线。  
3. 身份：仅本地用户 vs 尽快接 OIDC/SSO。  
4. M2 路由分类器：规则/小模型/LLM 判别的具体方案与成本。  
5. M3/M4 回归「不低于基线」的数值门槛与评测集规模。  
...
| 后续 Implementation Plan | 任务拆解、技术选型落地、排期 |

**下一步**：评审本 PRD → 锁定开放问题中的默认选型 → 编写实现计划（writing-plans）并开始 M1。
...
> 本文件为 brainstorming 归档副本；产品需求以仓库根目录 [`PRD.md`](../../../PRD.md) 为准。
